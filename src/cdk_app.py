...
class BedrockAgentStack(Stack):
    ...
    def _create_outputs(
        ...
        outputs = {
            ...
            "AggregationTableName": (database.aggregation_table.table_name, "DynamoDB Aggregation Table Name"),
            "LambdaArn": (lambdas.sqs_processor.function_arn, "SQS to DynamoDB Lambda Function ARN"),
            "StreamProcessorLambdaArn": (lambdas.stream_processor.function_arn, "DynamoDB Stream Processor Lambda ARN"),
            "BuildDeployAgentLambdaArn": (lambdas.build_deploy_agent.function_arn, "Build & Deploy Agent Lambda ARN"),
            ...
            "DeployEndpoint": (f"{api.api.url}deploy?tenantId=<TENANT_ID>", "Deploy Agent Endpoint"),
            "FrontendUrl": (f"https://{frontend.distribution.distribution_domain_name}", "Frontend Dashboard URL"),
            "FrontendBucketName": (frontend.bucket.bucket_name, "S3 Bucket for Frontend"),
        }
