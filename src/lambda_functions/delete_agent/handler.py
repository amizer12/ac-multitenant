...
def lambda_handler(event, context):
    ...
    try:
        ...
        print(f"Step 2: Deleting agent details from DynamoDB for tenantId: {tenant_id}, agentRuntimeId: {agent_runtime_id}")
        try:
            table.delete_item(Key={
                'tenantId': tenant_id,
                'agentRuntimeId': agent_runtime_id
            })
            print(f"Successfully deleted agent details from DynamoDB")
        except Exception as db_error:
            error_msg = str(db_error)
            print(f"Warning: Agent deleted from Bedrock but failed to delete from DynamoDB: {error_msg}")
